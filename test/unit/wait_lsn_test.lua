...
function g.test_zero_timeout()
    ...
    t.assert_equals(failover.wait_lsn(id, lsn,   0.1, 0), true)
    t.assert_equals(failover.wait_lsn(id, lsn+1, 0.1, 0), false)
...
function g.test_timings()
    ...
    local csw1 = fiber.info()[fiber.id()].csw
    t.assert_equals(failover.wait_lsn(id, lsn, 0.1, 1), true)
    local csw2 = fiber.info()[fiber.id()].csw
    t.assert_equals(csw1, csw2, 'Unnecessary yield')

    -- 2. False result shouldn't return until timeout expires
    local t0 = fiber.time()
    t.assert_equals(failover.wait_lsn(id, lsn+1, 0.1, 0.2), false)
    local t1 = fiber.time()
    ...
    local t0 = fiber.time()
    t.assert_equals(failover.wait_lsn(id, lsn+1, 0.01, 1), true)
    local t1 = fiber.time()
...
function g.test_absent_lsn()
    ...
    t.assert_equals(failover.wait_lsn(32, 0, 0.1, 0), true)
    t.assert_equals(failover.wait_lsn(32, 1, 0.1, 0), false)
end
