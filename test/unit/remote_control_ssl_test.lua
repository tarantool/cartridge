...
local g = t.group('ssl-remote-control')
...
function g.test_drop_connections()
    ...
    local conn_1 = assert(netbox.connect({uri='superuser:3.141592@127.0.0.1:13301',
        params={
            ...
            ssl_cert_file=CLIENT_CERT_FILE,
            ssl_key_file=CLIENT_KEY_FILE}}))
    local conn_2 = assert(netbox.connect({uri='superuser:3.141592@127.0.0.1:13301',
        params={
            transport='ssl',
            ssl_cert_file=CLIENT_CERT_FILE,
            ssl_key_file=CLIENT_KEY_FILE}}))
    ...
    local result = helpers.run_remotely({net_box = conn_1}, function()
        ...
        if helpers.tarantool_version_ge('3.5.0') then
            t.assert_str_matches(_G.conn_2.error, "Peer closed")
        else
    ...
    if helpers.tarantool_version_ge('3.5.0') then
        t.assert_equals(conn_1.error, 'Peer closed')
        t.assert_equals(conn_2.error, 'Peer closed')
    else
...
function g.test_late_accept()
    ...
    t.assert_equals(err, nil)
    t.assert_equals(ok, true)
    local url = 'superuser:3.141592@127.0.0.1:13301'

    local f = fiber.new(netbox.connect, {uri=url,
        params={
            transport='ssl',
            ssl_cert_file=CLIENT_CERT_FILE,
            ssl_key_file=CLIENT_KEY_FILE}})
    ...
    local conn_1 = netbox.connect({uri=url,
        params={
            transport='ssl',
            ssl_cert_file=CLIENT_CERT_FILE,
    ...
    --------------------------------------------------------------------
    local conn_4 = netbox.connect({uri=url, params={
        transport='ssl',
        ssl_cert_file=CLIENT_CERT_FILE,
        ssl_key_file=CLIENT_KEY_FILE}}, {wait_connected = false})
...
function g.test_bytestream()
    ...
    rc_start(13301)
    box.cfg({listen = box.NULL})
    local ctx = sslsocket.ctx(ffi.C.TLS_client_method())
    local rc = sslsocket.ctx_use_private_key_file(ctx, CLIENT_KEY_FILE)
    ...
    local conn_rc = sslsocket.tcp_connect('127.0.0.1', 13301, 10, ctx)
    check_conn(conn_rc)
    conn_rc:close()
    ...
    rc = sslsocket.ctx_use_certificate_file(ctx, CLIENT_CERT_FILE)
    assert(rc == true, 'Client certificate is invalid')
    local conn_box = sslsocket.tcp_connect('127.0.0.1', 13302, 10, ctx)
    check_conn(conn_box)
    conn_box:close()
end
...
function g.test_large_payload()
    ...
    local conn_box = assert(netbox.connect({uri='superuser:3.141592@127.0.0.1:13302', params={
        transport = 'ssl',
        ssl_ca_file = CA_FILE,
        ssl_cert_file = CLIENT_CERT_FILE,
        ssl_key_file = CLIENT_KEY_FILE,
    ...
    rc_start(13301)
    box.cfg({listen = box.NULL})
    local conn_rc = assert(netbox.connect({uri='superuser:3.141592@127.0.0.1:13301', params={
        transport = 'ssl',
        ssl_ca_file = CA_FILE,
        ssl_cert_file = CLIENT_CERT_FILE,
        ssl_key_file = CLIENT_KEY_FILE,
...
function g.test_async()
    rc_start(13301)
    local conn = assert(netbox.connect({uri='superuser:3.141592@127.0.0.1:13301', params={
        transport = 'ssl',
        ssl_ca_file = CA_FILE,
        ssl_cert_file = CLIENT_CERT_FILE,
        ssl_key_file = CLIENT_KEY_FILE,
...
function g.test_bad_username()
    ...
    rc_start(13301)
    box.cfg({listen = box.NULL})
    local conn_rc = assert(netbox.connect({uri='bad-user@127.0.0.1:13301', params={
        transport = 'ssl',
        ssl_ca_file = CA_FILE,
        ssl_cert_file = CLIENT_CERT_FILE,
        ssl_key_file = CLIENT_KEY_FILE,
...
function g.test_fd_cloexec()
    ...
    rc = sslsocket.ctx_use_certificate_file(ctx, CLIENT_CERT_FILE)
    assert(rc == true, 'Client certificate is invalid')
    local client = sslsocket.tcp_connect('127.0.0.1', 13301, 10, ctx)
