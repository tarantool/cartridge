...
const descriptionsByName = ({ fields = [] } = {}) =>
...
export function getInstanceData({ instanceUUID }) {
  return graphql
    .fetch(firstServerDetailsQuery, { uuid: instanceUUID })
    .then(
      ({
        servers,
        ...
        const {
          ...
          message,
          replicaset: {
            active_master: { uuid: activeMasterUUID },
...
export function refreshInstanceData({ instanceUUID }) {
  return graphql.fetch(nextServerDetailsQuery, { uuid: instanceUUID }).then(({ servers }) => ({
