...
describe('Deleting files', () => {
  ...
  it('deletes one file', () => {
    expect(reducer(state, deleteFile({ id: 'folder/file.ext' }))).toEqual([
      { path: 'file.ext' },
...
describe('Renaming', () => {
  describe('Renaming files', () => {
    const initialState = [{ path: 'file.ext' }, { path: 'folder/file.ext' }];

    it('renames file in root', () => {
      expect(reducer(initialState, renameFile({ id: 'file.ext', name: 'renamed.txt' }))).toMatchObject([
        { path: 'renamed.txt' },
        { path: 'folder/file.ext' },
      ]);
    });
    ...
    it('renames file in folder', () => {
      expect(reducer(initialState, renameFile({ id: 'folder/file.ext', name: 'renamed.txt' }))).toMatchObject([
        { path: 'file.ext' },
    ...
    it('can NOT rename a file to an empty name', () => {
      expect(reducer(initialState, renameFile({ id: 'dir', name: '' }))).toMatchObject(initialState);
    });
  });
  ...
  it('renames folder in root', () => {
    expect(reducer(initialState, renameFolder({ id: 'dir', name: 'renamedDir' }))).toMatchObject([
      { path: 'file.ext' },
  ...
  it('can NOT rename a folder to an empty name', () => {
    expect(reducer(initialState, renameFolder({ id: 'dir', name: '' }))).toMatchObject(initialState);
  });

  it('renames subfolder in folder', () => {
    expect(reducer(initialState, renameFolder({ id: 'dir/dir', name: 'renamedDir' }))).toMatchObject([
      { path: 'file.ext' },
      { path: 'dir' },
      { path: 'dir/file.ext' },
      { path: 'dir/renamedDir' },
  ...
  it('returns previous state when folder not found', () => {
    expect(reducer(initialState, renameFolder({ id: 'non/existing/dir', name: 'renamedDir' }))).toEqual(initialState);
  });
});
...
describe('Creating', () => {
  describe('Creating files', () => {
    ...
    describe('create a file in a folder', () => {
      ...
      it('can NOT create a file with an empty name', () => {
        expect(reducer(initialState, createFile({ name: '', parentPath: 'dir' }))).toEqual(initialState);
      });
    });
  ...
  describe('Creating folders', () => {
    ...
    it('can create a folder', () => {
      ...
      const newState = reducer(initialState, createFolder({ name: newName, parentPath: existingFolder }));
    ...
    it('can NOT create a folder with an empty name', () => {
      expect(reducer(initialState, createFolder({ name: '', parentPath: 'dir' }))).toEqual(initialState);
    });
  });
});
