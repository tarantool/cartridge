...
import { ConfirmModal, IconCancel, IconOk, Switcher, Text } from '@tarantool.io/ui-kit';
...
class AuthToggleButton extends React.Component<AuthToggleButtonProps, { visible: boolean }> {
  ...
  render() {
    const { implements_check_password, authorizationEnabled } = this.props;

    const { visible } = this.state;

    return implements_check_password ? (
      <React.Fragment>
        <Switcher className="meta-test__AuthToggle" onChange={this.showModal} checked={authorizationEnabled}>
          Auth
        </Switcher>
        <ConfirmModal
