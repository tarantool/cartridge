export const encodeURIComponent = (string) =>
  string.replace('%', '%25').replace('&', '%26').replace('/', '%2F').replace('=', '%3D');

export const decodeURIComponent = (string) =>
  string.replace('%25', '%').replace('%26', '&').replace('%2F', '/').replace('%3D', '=');
...
export function addSearchParams(search, params, castrated) {
  ...
  const query = Object.entries(parsed)
    .reduce((parts, entry) => {
      parts.push(entry.join('='));
      return parts;
    }, [])
    .join('&');
  return query ? (castrated ? query : `?${query}`) : '';
}
