...
local function validate_schema_labels(field, object)
    ...
    for label_name, label_value in pairs(object.labels) do
        ...
        e_label_config:assert(type(label_name) == "string", "%s must be string", tostring(label_name_field))
        ...
        e_label_config:assert(type(label_value) == "string", "%s must be string", label_value_field, tostring(label_name))

        e_label_config:assert(label_value:len() <= 63, "%s must not exceed 63 character", label_value_field, label_name)
        ...
        e_label_config:assert(label_value:sub(0, 1) ~= ".", "%s must not start with dot (.)", label_value_field, label_name)

        e_label_config:assert(label_value:sub(-1) ~= ".", "%s must not end with dot (.)", label_value_field, label_name)
    end
